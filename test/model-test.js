...
test('should properly fetch from the API and translate features', (t) => {
  ...
  model.getData({}, (err, geojson) => {
    ...
    t.deepEqual(
      ...
      [-122.675109, 45.5003833],
      'translates geometry correctly',
    );
    t.ok(feature.properties, 'creates attributes');
    t.equal(
      feature.properties.expires,
      new Date(1484268019000).toISOString(),
      'translates expires field correctly',
    );
    t.equal(
      feature.properties.expires,
      new Date(1484268019000).toISOString(),
      'translates serviceDate field correctly',
    );
    t.equal(
      feature.properties.expires,
      new Date(1484268019000).toISOString(),
      'translates time field correctly',
    );
    t.end();
  });
});
